<?php 
	...
	*/
	class Saldo_m extends CI_Model
	{
		...
		{
			return $query = $this->db->query("SELECT * FROM saldo where MONTH(date) = ".$bulan." ORDER BY id DESC")->result();
		}
		...
		public function getSaldoByDept($kode_dept = null)
		{
			if($kode_dept != null)
			{
				$query = $this->db->query("SELECT s.*, p.kode_dept FROM pengajuan as p LEFT JOIN saldo as s ON p.no_pengajuan=s.no_bukti WHERE kode_dept = ".$kode_dept." AND p.status = 4 AND s.no_bukti IS NOT NULL ORDER BY s.id DESC")->result();

				return $query;
			}else
			{
				$query = $this->db->query("SELECT s.*, p.kode_dept FROM pengajuan as p JOIN saldo as s ON p.no_pengajuan=s.no_bukti AND p.status = 4 AND s.no_bukti IS NOT NULL ORDER BY s.id DESC")->result();
